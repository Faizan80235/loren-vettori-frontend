...
//   const priceInfo = getPriceInfo();
//   const isMadeToMeasureAvailable = supportsMadeToMeasure();
//   const isJacket = productData.subcategory === 'Jackets';

//   return (
//     <div className="border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100">
//       {/* Size Guide Modal - ONLY FOR JACKETS */}
//       {showSizeGuide && isJacket && (
//         <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onClick={() => setShowSizeGuide(false)}>
//           <div className="bg-white rounded-lg max-w-3xl w-full max-h-[90vh] overflow-auto relative" onClick={(e) => e.stopPropagation()}>
//             <button 
//               onClick={() => setShowSizeGuide(false)}
...
//           {/* Size Selection with Size Guide (Size Guide button only for Jackets) */}
//           {availableSizes.length > 0 && (
//             <div className="flex flex-col gap-4 my-8">
//               <div className="flex items-center justify-between">
//                 <p className="font-medium">Select Size</p>
...
const Product = () => {
  ...
  // Get color variant image (from product's actual images)
  const getColorVariantImage = (variant) => {
    if (!variant.images || variant.images.length === 0) {
      return 'https://via.placeholder.com/80x80/CCCCCC/CCCCCC?text=No+Image';
    }
    ...
    return getImageUrl(primaryImage.url || primaryImage.filename || primaryImage);
  };

  // Find color variants (products with exact same name)
  const findColorVariants = (currentProduct) => {
    if (!currentProduct || !products) {
      console.log('No product or products array');
      return [];
    }
    
    // Get exact product name (case-insensitive)
    const currentName = currentProduct.name.trim().toLowerCase();
    
    console.log('🔍 Searching variants for:', currentName);
    
    // Find all products with exact same name (excluding current)
    const variants = products.filter(p => {
      if (p._id === currentProduct._id) return false;
      
      // Match exact name (case-insensitive)
      const isSameName = p.name.trim().toLowerCase() === currentName;
      
      if (isSameName) {
        console.log('✅ Found variant:', p.name, '- ID:', p._id);
      }
    ...
    // Only return variants if there are other products with same name
    if (variants.length === 0) {
      console.log('❌ No variants found');
      return [];
    }
    
    // Create variants array with current product first
    const allVariants = [
      { 
        ...currentProduct, 
        isCurrentProduct: true,
        displayColor: currentProduct.color || currentProduct.name,
    ...
    variants.forEach((variant, index) => {
      allVariants.push({
        ...
        displayColor: variant.color || variant.name,
        displayName: variant.name
      });
    });
    
    console.log('🎨 Final variants array:', allVariants.length);
  ...
  const fetchProductData = async () => {
    try {
      ...
      if (product) {
        ...
        // Set current product color
        if (product.color) {
          setSelectedColor(product.color);
        }
        
        // Find color variants
        const variants = findColorVariants(product);
  ...
  const handleColorChange = (variantProduct) => {
    console.log('🔄 Switching to variant:', variantProduct._id);
    // Navigate to the new color variant product
    navigate(`/product/${variantProduct._id}`);
  };
  ...
  const proceedToAddToCart = async () => {
    try {
      for (let i = 0; i < quantity; i++) {
        addToCart(productData._id, size || 'default', null, madeToMeasure);
      }
    } catch (error) {
      console.error('Error adding to cart:', error);
      toast.error('Failed to add item to cart');
    ...
    <div className="border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100">
      ...
      <div className="flex flex-col sm:flex-row gap-12">
        ...
        <div className="flex-1">
          ...
          {/* Color Variants - Show only if multiple products with same name exist */}
          {colorVariants.length > 1 && (
            <div className="flex flex-col gap-4 my-8 border-t pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {colorVariants.length} color{colorVariants.length > 1 ? 's' : ''} available
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                {colorVariants.map((variant) => (
                  <div 
                    key={variant._id} 
                    onClick={() => !variant.isCurrentProduct && handleColorChange(variant)} 
                    className={`group cursor-pointer transition-all duration-200 ${
                      variant.isCurrentProduct ? 'opacity-100' : 'opacity-70 hover:opacity-100'
                    }`}
                    title={variant.displayColor}
                  >
                    <div className={`relative w-20 h-20 rounded-lg border-2 overflow-hidden transition-all duration-200 ${
                      ...
                      <img 
                        src={getColorVariantImage(variant)}
                        alt={variant.displayColor}
                        className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300"
                        onError={(e) => { e.target.src = 'https://via.placeholder.com/80x80/CCCCCC/CCCCCC?text=Error'; }}
                      />
                      {variant.isCurrentProduct && (
                    ...
                    <p className={`text-xs text-center mt-1.5 capitalize ${
                      variant.isCurrentProduct ? 'font-semibold text-gray-900' : 'text-gray-600'
                    }`}>
